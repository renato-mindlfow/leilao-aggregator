"""Validação rápida do LOTE 2 - Sites grandes conhecidos"""
import httpx
import json
...
print("="*70)
print("VALIDAÇÃO LOTE 2 - Sites grandes conhecidos")
print("="*70)
...
for site in SITES:
    ...
    if portal_results:
        ...
        print(f"  [OK] API encontrada! Portal ID: {portal_id}, Total: {total} imoveis")
        ...
        print(f"  [WARN] Nenhuma API encontrada ou sem imoveis")
        # Desabilitar
        config_file = CONFIG_DIR / f"{site['id']}.json"
        with open(config_file, 'r', encoding='utf-8') as f:
            config = json.load(f)
        ...
        config['notes'] = ["Nenhuma API Superbid encontrada com imoveis - REQUER ANALISE MANUAL"]
...
print("RESUMO")
print("="*70)
print(f"{'Site':<25} {'Tem Imoveis?':<15} {'URL':<10} {'Metodo':<12} {'Imoveis':<10} {'Status'}")
print("-"*70)
