"""
Leilões Gold Scraper
Scrapes real estate properties from leiloesgold.com.br
...
class LeiloesGoldScraper:
    """Scraper for Leilões Gold website using Playwright with Stealth."""
    
    BASE_URL = "https://www.leiloesgold.com.br"
    IMOVEIS_URL = f"{BASE_URL}/imoveis"
    AUCTIONEER_ID = "leiloes_gold"
    ...
    def scrape_properties(self, max_properties: Optional[int] = None) -> List[Dict]:
        """Scrape properties from Leilões Gold."""
    ...
    async def _scrape_properties_async(self, max_properties: Optional[int] = None) -> List[Dict]:
        """Scrape properties from Leilões Gold (async version)."""
        logger.info(f"Iniciando scraping do Leilões Gold (max {max_properties} imóveis)...")
    ...
    async def _scrape_property_page(self, url: str) -> Optional[Dict]:
        ...
        try:
            ...
            # Extrair imagem
            try:
                img_elem = await self.page.query_selector("img[src*='leiloesgold'], .property-image img, img.property-image")
                if img_elem:
...
if __name__ == "__main__":
    scraper = LeiloesGoldScraper(headless=True)
