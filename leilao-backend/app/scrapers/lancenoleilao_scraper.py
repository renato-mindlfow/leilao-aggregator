"""
Lance no Leilão Scraper
Scrapes real estate properties from lancenoleilao.com.br
...
class LanceNoLeilaoScraper:
    """Scraper for Lance no Leilão website using Playwright with Stealth."""
    
    BASE_URL = "https://www.lancenoleilao.com.br"
    IMOVEIS_URL = f"{BASE_URL}/imoveis"
    AUCTIONEER_ID = "lance_no_leilao"
    ...
    def scrape_properties(self, max_properties: Optional[int] = None) -> List[Dict]:
        """Scrape properties from Lance no Leilão."""
    ...
    async def _scrape_properties_async(self, max_properties: Optional[int] = None) -> List[Dict]:
        """Scrape properties from Lance no Leilão (async version)."""
        logger.info(f"Iniciando scraping do Lance no Leilão (max {max_properties} imóveis)...")
    ...
    async def _scrape_property_page(self, url: str) -> Optional[Dict]:
        ...
        try:
            ...
            property_data = {
                ...
                'url': url,
                'auctioneer_url': url,
                'auctioneer_name': 'Lance no Leilão',
                'auctioneer_id': self.AUCTIONEER_ID,
            }
            ...
            # Extrair imagem
            try:
                img_elem = await self.page.query_selector("img[src*='lancenoleilao'], .property-image img")
                if img_elem:
...
if __name__ == "__main__":
    scraper = LanceNoLeilaoScraper(headless=True)
