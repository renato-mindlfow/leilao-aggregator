"""
Leilão Brasil Scraper
Scrapes real estate properties from leilaobrasil.com.br
...
class LeilaoBrasilScraper:
    """Scraper for Leilão Brasil website using Playwright with Stealth."""
    
    BASE_URL = "https://www.leilaobrasil.com.br"
    IMOVEIS_URL = f"{BASE_URL}/imoveis"
    AUCTIONEER_ID = "leilao_brasil"
    ...
    def scrape_properties(self, max_properties: Optional[int] = None) -> List[Dict]:
        """Scrape properties from Leilão Brasil."""
    ...
    async def _scrape_properties_async(self, max_properties: Optional[int] = None) -> List[Dict]:
        """Scrape properties from Leilão Brasil (async version)."""
        logger.info(f"Iniciando scraping do Leilão Brasil (max {max_properties} imóveis)...")
    ...
    async def _scrape_property_page(self, url: str) -> Optional[Dict]:
        ...
        try:
            ...
            # Extrair imagem
            try:
                img_elem = await self.page.query_selector("img[src*='leilaobrasil'], .property-image img")
                if img_elem:
...
if __name__ == "__main__":
    scraper = LeilaoBrasilScraper(headless=True)
