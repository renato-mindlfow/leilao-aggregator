"""
JE Leilões Scraper
Scrapes real estate properties from jeleiloes.com.br
...
class JELeiloesScraper:
    """Scraper for JE Leilões website using Playwright with Stealth."""
    
    BASE_URL = "https://www.jeleiloes.com.br"
    IMOVEIS_URL = f"{BASE_URL}/imoveis"
    AUCTIONEER_ID = "je_leiloes"
    ...
    def scrape_properties(self, max_properties: Optional[int] = None) -> List[Dict]:
        """Scrape properties from JE Leilões."""
    ...
    async def _scrape_properties_async(self, max_properties: Optional[int] = None) -> List[Dict]:
        """Scrape properties from JE Leilões (async version)."""
        logger.info(f"Iniciando scraping do JE Leilões (max {max_properties} imóveis)...")
    ...
    async def _scrape_property_page(self, url: str) -> Optional[Dict]:
        ...
        try:
            ...
            # Extrair imagem
            try:
                img_elem = await self.page.query_selector("img[src*='jeleiloes'], .property-image img")
                if img_elem:
...
if __name__ == "__main__":
    scraper = JELeiloesScraper(headless=True)
