# TAREFA AUTÔNOMA: Testar e Corrigir Todos os Leiloeiros

## INSTRUÇÕES CRÍTICAS

**ESTA TAREFA DEVE RODAR DE FORMA 100% AUTÔNOMA**
- NÃO pare para perguntar NADA
- NÃO peça confirmação
- Execute TUDO sequencialmente
- Se algo falhar, REGISTRE e CONTINUE
- Gere relatório final ao terminar
...
mkdir -p logs\scraper_audit
```

### 1.2 Criar Script de Auditoria Completa

Criar arquivo `scripts/auditoria_completa_leiloeiros.py`:
    ...
    async def _testar_url(self, url: str, leiloeiro_id: str, leiloeiro_name: str) -> Tuple[bool, int, str]:
        ...
        try:
            ...
            if properties and len(properties) > 0:
                logger.info(f"  ✅ SUCESSO: {len(properties)} imóveis")
                return True, len(properties), url
            else:
                logger.warning(f"  ⚠️ Sem imóveis: {url}")
...
"""
    ...
    def _imprimir_resumo(self, relatorio: Dict):
        ...
        print(f"⚠️ Falha: {r['falhas']}")
        print(f"❌ Erro: {r['erros_criticos']}")
        print(f"🏠 Total imóveis: {r['total_imoveis']}")
        print(f"⏱️ Duração: {relatorio['meta']['duracao_minutos']:.1f} min")
        
        print("\n" + "-"*60)
        print("LEILOEIROS FUNCIONANDO:")
        print("-"*60)
        for l in relatorio["leiloeiros_funcionando"]:
            print(f"  ✅ {l['name']}: {l['imoveis']} imóveis")
        ...
        if r["taxa_sucesso"] >= 70:
            ...
            print("\n" + "="*60)
            print(f"⚠️ META NÃO ATINGIDA ({r['taxa_sucesso']}% < 70%)")
            print("="*60)
...
async def main():
    ...
    print("   Você pode sair, o processo continuará rodando.\n")
    ...
    try:
        ...
        if relatorio["resumo"]["taxa_sucesso"] >= 70:
            ...
            print("\n⚠️ AUDITORIA CONCLUÍDA - META NÃO ATINGIDA")
            sys.exit(1)
...
## FASE 2: EXECUTAR AUDITORIA (30-60 min)
...
**NÃO INTERROMPA** - deixe rodar até o final.

---

## FASE 3: ANALISAR RESULTADOS E APLICAR CORREÇÕES

Após a auditoria terminar, analisar os resultados em `logs/scraper_audit/`.

### 3.1 Se taxa >= 70%: SUCESSO!

```bash
git add scripts/auditoria_completa_leiloeiros.py
git add logs/scraper_audit/
git commit -m "feat: Adicionar auditoria completa de leiloeiros

- Script de auditoria autônoma
- Testados X leiloeiros
- Taxa de sucesso: Y%
- Relatórios em logs/scraper_audit/"
git push
```

### 3.2 Se taxa < 70%: Identificar Padrões de Falha

Ler o relatório JSON e identificar padrões:

1. **Timeout** → Sites pesados, precisa de scraper específico
2. **DNS Error** → URL errada, corrigir
3. **Bloqueio** → Site tem proteção forte
4. **Sem imóveis** → URL de busca incorreta

Para cada padrão, aplicar correção:

**URLs erradas:**
- Buscar URL correta no navegador
- Atualizar LEILOEIROS_MESTRE no script

**Sites com bloqueio:**
- Adicionar à lista de "requer scraper específico"
- Esses usarão os scrapers Selenium/Playwright dedicados

---

## FASE 4: GERAR RELATÓRIO FINAL

Criar arquivo `AUDITORIA_FINAL.md` com:
...
- Taxa de sucesso: X%
- Leiloeiros funcionando: Y
- Leiloeiros com problemas: Z

## URLs Validadas (para uso em produção)

[Lista de URLs que funcionaram]

## Leiloeiros que Requerem Scraper Específico

[Lista de leiloeiros com proteção forte]
...
```bash
git add -A
git commit -m "audit: Auditoria completa de leiloeiros concluída

Resultado: X% de sucesso
- Y leiloeiros funcionando com LLMEnhancedScraper
- Z leiloeiros requerem scraper específico

Detalhes em logs/scraper_audit/"
git push
```
...
- [ ] Relatório gerado em `logs/scraper_audit/`
- [ ] Commit realizado com resultados
- [ ] (Bônus) Taxa >= 70% atingida
...
2. **REGISTRE** todos os erros mas continue executando
3. **GERE** relatórios detalhados para análise posterior
4. **COMMIT** os resultados independente do sucesso

Esta tarefa pode levar 1-2 horas para completar.
