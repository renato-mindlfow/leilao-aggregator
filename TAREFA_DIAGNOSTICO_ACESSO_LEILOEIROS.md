# 🔬 TAREFA: DIAGNÓSTICO DE PROBLEMAS DE ACESSO AOS LEILOEIROS

**Objetivo**: Identificar a causa raiz de 72.7% dos sites marcados como "OFFLINE"
**Hipótese**: Não são sites offline, mas sim bloqueios de bot ou problemas de detecção

**Tempo Estimado**: 1-2 horas
**Abordagem**: Testes em camadas progressivas

---

## 📋 METODOLOGIA

Testar **20 sites amostrais** com **6 métodos diferentes** para identificar:
1. Qual método funciona melhor
2. Qual é a causa real do "OFFLINE"
3. Qual solução implementar na Fase 2

### Sites Amostrais (mix de conhecidos + "offline")

```python
SITES_TESTE = [
    # GRUPO A: Sites que SABEMOS que funcionam (controle)
    ...
    # GRUPO B: Sites marcados como OFFLINE (testar se realmente são)
    ...
    # GRUPO C: Sites com property_count > 0 mas status=error
    ...
    # GRUPO D: Sites aleatórios para variedade
    ("Superbid", "https://www.superbid.net"),
    ("Zukerman", "https://www.zfrancaleiloes.com.br"),
...
## 🔧 ETAPA 1: CRIAR SCRIPT DE DIAGNÓSTICO

**Arquivo**: `leilao-backend/scripts/diagnostico_acesso.py`
...
def gerar_resumo_console(resultados: List[SiteDiagnostico]):
    ...
    cloudflare = len([k for k in por_diagnostico if "CLOUDFLARE" in k])
...
mkdir -p logs/diagnostico_acesso

# Executar diagnóstico
python scripts/diagnostico_acesso.py
```

**Tempo estimado**: 20-30 minutos para 20 sites
...
1. **`logs/diagnostico_acesso/RELATORIO_DIAGNOSTICO_*.md`** - Relatório visual
2. **`logs/diagnostico_acesso/diagnostico_*.json`** - Dados estruturados
3. **Screenshots** de sites testados com Playwright Headed

### Possíveis Diagnósticos

| Diagnóstico | Significado | Solução |
|-------------|-------------|---------|
| `SITE_OK` | Funciona normalmente | ✅ Nenhuma ação |
| `REQUER_JAVASCRIPT` | Site é SPA/dinâmico | Usar Playwright |
| `CLOUDFLARE_BYPASS_OK` | Cloudflare mas stealth funciona | Usar Playwright Stealth |
| `CLOUDFLARE_BLOQUEIO` | Cloudflare bloqueia tudo | Proxy residencial |
| `CAPTCHA_BLOQUEIO` | Requer CAPTCHA | Serviço de solving |
| `HEADLESS_DETECTADO` | Detecta browser automático | Undetected Chrome |
| `ANTI_BOT_FORTE` | Proteção avançada | Proxy + Stealth avançado |
| `SITE_INACESSIVEL` | Realmente offline | Remover da lista |
| `DNS_FALHOU` | Domínio não existe | Remover da lista |

---

## ✅ CRITÉRIOS DE SUCESSO

1. **Identificar causa raiz** dos 72.7% "OFFLINE"
2. **Quantificar** quantos sites:
   - Estão realmente offline
   - Requerem JavaScript
   - Têm Cloudflare
   - Têm anti-bot
3. **Definir estratégia** baseada nos dados reais
...
| Se a maioria for... | Ação |
|---------------------|------|
| `REQUER_JAVASCRIPT` | Usar Playwright Stealth como padrão |
| `CLOUDFLARE_*` | Implementar proxy residencial |
| `HEADLESS_DETECTADO` | Usar undetected-chromedriver |
| `SITE_INACESSIVEL` | Limpar lista de leiloeiros inativos |
| Mix variado | Implementar fallback em camadas |
